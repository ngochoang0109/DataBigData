A = LOAD './apache_pig/data/tstat-sample.txt'  using PigStorage(' ') AS (ip_c:chararray,
port_c:int,
packets_c:int,
rst_c:int,
ack_c:int,
...
B = FOREACH Load_Data GENERATE ip_c, data_bytes_c;

Group_IP_C = GROUP B by ip_c;

Result= FOREACH Group_IP_C GENERATE group as ip_user, SUM(B.data_bytes_c) as sum_data;

Sort = ORDER Result BY sum_data DESC;

Get_100_User = LIMIT Sort 100;
